...
- 마땅한 규칙이 생각나지 않아, 테두리의 숫자들 실제로 회전시켜주는 것을 구현했습니다.
- int[][] arr로 행렬 선언하고, rotate()로 회전한 후 최솟값을 반환하도록 했습니다.
- rotate()
    - 최종적으로 return할 min값을 선언해줍니다.
    - temp : 하나씩 바꾸면 맨 처음에 바뀐 대상이 되는 (r1, c1)이 바꿀 값(r1 + 1, c1)은 따로 저장해줍니다.
    - 반복문을 돌며 (r1, c1 + 1)부터 (r1 + 1, c1)까지 값을 회전시킴과 동시에, min 값을 갱신합니다.
- next()
    - 회전하는 테두리의 좌표에서 다음 좌표를 알려주는 메서드입니다.
 */
